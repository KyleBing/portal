 ...
 * 烹饪食谱表
 */
export interface CookingRecipeNew {
  id?: number; // 主键ID
  name: string; // 食谱名称（中文）
 ...
 * 制作表
 */
export interface CraftNew {
  id?: number; // 主键ID
  name: string; // 物品名称（中文）
  name_en: string; // 物品名称（英文）
  sortid?: number | null; // 排序ID
  version: VersionNew[]; // 游戏版本 可能属于多个版本
  tab: CraftTabNew[]; // 制作标签  可能属于多个标签
 ...
 * 版本表
 */
export interface VersionNew {
  id?: number; // 主键ID
  code: string; // 版本代码
  name: string; // 版本名称
  name_en?: string | null; // 英文名称
