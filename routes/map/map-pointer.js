...
const routerMapPointer = express.Router()
...
routerMapPointer.post('/list', (req: Request, res: Response, next) => {
...
routerMapPointer.get('/detail', (req: Request, res: Response, next) => {
    let sql = `select  
                                ${CURRENT_TABLE}.id, 
                                ${CURRENT_TABLE}.name, 
                                ${CURRENT_TABLE}.pointers, 
...
routerMapPointer.post('/add', (req: Request, res: Response, next) => {
    // 1. 验证用户信息是否正确
    verifyAuthorization(req)
        .then(userInfo => {
            // 2. 检查路线名是否已存在
...
routerMapPointer.put('/modify', (req: Request, res: Response, next) => {
...
routerMapPointer.delete('/delete', (req: Request, res: Response, next) => {
    Promise.all([
        verifyAuthorization(req),
        getDataFromDB('diary', [`select * from ${CURRENT_TABLE} where id = ${req.body.id}`], true)
    ])
