...
/* GET users listing. */
routerUser.post('/register', (req: Request, res: Response, next) => {
    // TODO: 验证传过来的数据库必填项
    if (req.body.invitationCode === CONFIG_PROJECT.invitation){ // 万能全局邀请码
        registerUser(req, res)
    } else {
        (getDataFromDB('diary', [`select * from invitations where id = '${req.body.invitationCode}'`], true) as Promise<{binding_uid: string}>)
            .then(result => {
                if (result){
...
// 检查用户名或邮箱是否存在
function checkEmailOrUserNameExist(email, username): Promise<>{
    let sqlArray = []
    sqlArray.push(`select * from users where email='${email}' or username ='${username}'`)
    return getDataFromDB( 'diary', sqlArray)
}

routerUser.post('/list', (req: Request, res: Response, next) => {
...
routerUser.get('/detail', (req: Request, res: Response, next) => {
    let sqlArray = []
...
routerUser.get('/avatar', (req: Request, res: Response, next) => {
    let sqlArray = []
...
routerUser.post('/add', (req: Request, res: Response, next) => {
    checkEmailOrUserNameExist(req.body.email, req.body.username)
        .then(dataEmailExistArray => {
            // email 记录是否已经存在
            if (dataEmailExistArray.length > 0){
...
routerUser.put('/set-profile', (req: Request, res: Response, next) => {
    // 1. 验证用户信息是否正确
    verifyAuthorization(req)
        .then(userInfo => {
...
routerUser.put('/modify', (req: Request, res: Response, next) => {
...
routerUser.delete('/delete', (req: Request, res: Response, next) => {
    // 1. 验证用户信息是否正确
    verifyAuthorization(req)
        .then(userInfo => {
            if (userInfo.group_id === 1){
...
routerUser.post('/login', (req: Request, res: Response, next) => {
    let sqlArray = []
...
// 修改密码
routerUser.put('/change-password', (req: Request, res: Response, next) => {
...
// 注销帐号
routerUser.delete('/destroy-account', (req: Request, res: Response, next) => {
    verifyAuthorization(req)
        .then(userInfo => {
            // 演示帐户时不允许执行注销操作
            if (userInfo.email === 'test@163.com'){
