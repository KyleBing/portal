...
const WubiDict_1 = require("../../src/entity/WubiDict");
...
router.post('/upload-dict', uploadStorage.single('dict'), (req, res) => {
    ...
    (0, utility_1.verifyAuthorization)(req)
        .then(userInfo => {
        let dict = new WubiDict_1.WubiDict(req.file.buffer.toString(), 'temp', 'temp');
        let sqlArray = [];
        ...
        (0, utility_1.getDataFromDB)('diary', sqlArray)
            ...
            .catch(err => {
            res.send(new Response_1.ResponseError(err, '导入词条失败'));
        });
    })
        .catch(err => {
...
router.post('/export-extra', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.getDataFromDB)('diary', [sqlBase], false)
            ...
            res.send(new Response_1.ResponseSuccess(wordList, '请求成功'));
        });
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
...
router.post('/check-exist', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.getDataFromDB)('diary', sqlArray, false)
            ...
            .catch(err => {
            res.send(new Response_1.ResponseError(err, '查询失败'));
        });
    })
        .catch(err => {
        res.send(new Response_1.ResponseError(err, '无权操作'));
...
router.post('/add', (req, res) => {
    ...
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.getDataFromDB)('diary', sqlArray)
            ...
            .catch(err => {
            res.send(new Response_1.ResponseError(err, '添加失败'));
        });
    })
        .catch(err => {
        res.send(new Response_1.ResponseError(err, '无权操作'));
        ...
        Promise.all(req.body.words.map(word => {
            ...
            if (insertWords.length === 0) {
                ...
                let sqlArray = insertWords.map(word => {
                    let parseWord = (0, utility_1.unicodeEncode)(word.word); // !
                    let timeNow = (0, utility_1.dateFormatter)(new Date());
                    let isApproved = userInfo.group_id === 1 ? 1 : 0; // 管理员添加的词条默认就是已经 approved
