...
exports.WubiDict = void 0;
// 字典对象
const WubiWord_1 = __importDefault(require("./WubiWord"));
const WubiWordGroup_1 = __importDefault(require("./WubiWordGroup"));
...
class WubiDict {
    ...
    getDictWordsInGroupMode(fileContent) {
        ...
        lines.forEach((item, index) => {
            if (item.startsWith('##')) { // 注释分组
                ...
                temp = new WubiWordGroup_1.default(this.lastGroupIndex++, item.substring(3).trim());
                lastItemIsEmptyLine = false;
            }
            else if (item.indexOf('\t') > -1) { // 是词条
                if (!temp) { // 第一行是词条时，没有分组名时
                    temp = new WubiWordGroup_1.default(this.lastGroupIndex++);
                }
    ...
    getRepetitionWords(filterSingleCharacter, isWithAllRepeatWord) {
        ...
        if (this.isGroupMode) {
            ...
            repetitionWords.push(new WubiWordGroup_1.default(999, '重复的词条', groupRepeatedWords));
        }
        else {
                ...
                if (filterSingleCharacter) {
                    if (wordMap.has(word.word) && getUnicodeStringLength(word.word) === 1) {
                        ...
                        if (isWithAllRepeatWord) {
                            let matchedWord = wordMap.get(word.word);
                            if (matchedWord)
                                repetitionWords.push(matchedWord);
                        }
                    }
                    else { // 如果 map 中没有这个词的记录，添加这个记录
                        wordMap.set(word.word, word);
    ...
    addNewWord(word, groupIndex) {
        if (this.isGroupMode) {
            if (groupIndex !== -1) {
                ...
                let newWordGroup = new WubiWordGroup_1.default(this.lastGroupIndex++, '- 未命名 -', [word]);
    ...
    // 删除词条
    deleteWords(wordIdSet, isDeleteInSelf) {
        if (this.isGroupMode) {
    ...
    addGroupBeforeId(groupIndex) {
        this.wordsOrigin.splice(groupIndex, 0, new WubiWordGroup_1.default(this.lastGroupIndex++, '', [], true));
    }
    // 分组模式：删除分组
    deleteGroup(groupId) {
    ...
    toYamlString() {
        ...
        if (this.isGroupMode) {
            this.wordsOrigin.forEach(group => {
                let tempGroupString = '';
                tempGroupString = tempGroupString + `## ${group.groupName}${os_1.default.EOL}`; // + groupName
                group.dict.forEach(item => {
                    tempGroupString = tempGroupString + item.toYamlString() + os_1.default.EOL;
                });
                yamlBody = yamlBody + tempGroupString + os_1.default.EOL; // 每组的末尾加个空行
            });
            return this.header + os_1.default.EOL + yamlBody;
        }
        else {
            let yamlBody = '';
            this.wordsOrigin.forEach(item => {
                yamlBody = yamlBody + item.toYamlString() + os_1.default.EOL;
            });
            return this.header + os_1.default.EOL + yamlBody;
        }
    }
...
function getWordFromLine(index, lineStr) {
    ...
    return new WubiWord_1.default(index, code, word, priority, note);
}
