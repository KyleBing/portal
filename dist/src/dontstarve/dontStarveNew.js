...
// 辅助函数：获取版本ID（通过版本代码）
function getVersionId(versionCode) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = yield (0, utility_1.getDataFromDB)(DB_NAME, [
...
function getEntityVersions(tableName, entityId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const results = yield (0, utility_1.getDataFromDB)(DB_NAME, [
...
function getBatchEntityVersions(tableName, entityIds) {
    return __awaiter(this, void 0, void 0, function* () {
...
function getBatchCraftTabs(craftIds) {
    return __awaiter(this, void 0, void 0, function* () {
...
function enrichListData(tableName, dataList) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (['characters', 'mobs', 'plants', 'things', 'crafts'].includes(tableName) && entityIds.length > 0) {
            versionMap = yield getBatchEntityVersions(tableName, entityIds);
        }
        if (tableName === 'crafts' && entityIds.length > 0) {
            tabMap = yield getBatchCraftTabs(entityIds);
        }
        // 组装数据
...
// get list
function getDataList(tableName, path, searchFields, hasVersion, hasTab) {
    router.get(path, (req, res) => {
        ...
        if (req.query.keyword && searchFields && searchFields.length > 0) {
            ...
            let keywordConditions = searchFields.map(field => `${tableName}.${field} LIKE '%${keyword}%' ESCAPE '/'`).join(' OR ');
        ...
        if (usePagination) {
            ...
            let promisesAll = [];
            // 查询列表数据
            promisesAll.push((0, utility_1.getDataFromDB)(DB_NAME, [
            ...
            Promise.all(promisesAll)
                .then((_a) => __awaiter(this, [_a], void 0, function* ([dataList, dataSum]) {
                // 处理数据，添加版本和标签数组
                const enrichedList = yield enrichListData(tableName, dataList || []);
            ...
            // 不使用分页：返回所有数据
            (0, utility_1.getDataFromDB)(DB_NAME, [
                `SELECT ${selectFields} FROM ${tableName} ${joinClause} ${whereClause}`
            ])
                .then((data) => __awaiter(this, void 0, void 0, function* () {
                if (data) {
                    // 处理数据，添加版本和标签数组
                    const enrichedList = yield enrichListData(tableName, data);
                    res.send(new Response_1.ResponseSuccess(enrichedList));
                }
                else {
...
function getDataInfo(tableName, path, hasVersion) {
    router.get(path, (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const data = yield (0, utility_1.getDataFromDB)(DB_NAME, [`SELECT ${selectFields} FROM ${tableName} ${joinClause} WHERE ${tableName}.id = ${req.query.id}`], true);
            if (data) {
                ...
                if (hasVersion && data.id) {
                    enriched.version = yield getEntityVersions(tableName, data.id);
                }
                // 对于crafts，添加标签数组
                if (tableName === 'crafts' && data.id) {
                    enriched.tab = yield getCraftTabs(data.id);
                }
                // 移除临时的version字段（如果存在）
                delete enriched.version_name;
                delete enriched.version_name_en;
                res.send(new Response_1.ResponseSuccess(enriched));
            }
            else {
                res.send(new Response_1.ResponseError('', '无数据'));
            }
...
router.post('/character/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        const versionIds = Array.isArray(req.body.version)
            ? req.body.version
            : [req.body.version].filter(v => v != null);
        if (versionIds.length === 0) {
            res.send(new Response_1.ResponseError('', '版本不能为空'));
            return;
        }
        let sqlArray = [];
        // 插入角色数据
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, sqlArray)
            ...
            if (!characterId) {
                res.send(new Response_1.ResponseError('', '插入失败'));
                return;
            }
            // 插入所有版本关系（第一个作为主版本）
            let relationSqls = [];
            ...
            (0, utility_1.getDataFromDB)(DB_NAME, relationSqls)
                .then(() => {
                (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
                res.send(new Response_1.ResponseSuccess({ id: characterId }, '添加成功'));
            })
                .catch(err => {
                res.send(new Response_1.ResponseError(err, '添加版本关系失败'));
            });
        }))
            .catch(err => {
            res.send(new Response_1.ResponseError(err, err.message));
...
router.post('/cookingrecipe/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (!req.body.name || !req.body.name_en) {
            res.send(new Response_1.ResponseError('', '名称和英文名称不能为空'));
            return;
        }
        let sqlArray = [];
        sqlArray.push(`
...
router.post('/craft/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (!req.body.name || !req.body.name_en || !req.body.crafting) {
            res.send(new Response_1.ResponseError('', '名称、英文名称和制作材料不能为空'));
            return;
        }
        // 处理版本数组：支持 number[] 或单个 number
        const versionIds = req.body.version
            ? (Array.isArray(req.body.version) ? req.body.version : [req.body.version]).filter(v => v != null)
            : [];
        // 处理标签数组：支持 number[] 或单个 number
        const tabIds = req.body.tab
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, sqlArray)
            ...
            if (relationSqls.length > 0) {
                (0, utility_1.getDataFromDB)(DB_NAME, relationSqls)
                    .then(() => {
                    (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
                    res.send(new Response_1.ResponseSuccess({ id: craftId }, '添加成功'));
                })
                    .catch(err => {
                    res.send(new Response_1.ResponseError(err, '添加关系失败'));
                });
            }
            else {
                (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
...
router.put('/craft/modify', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (req.body.tab !== undefined) {
            const tabIds = Array.isArray(req.body.tab)
                ? req.body.tab
...
router.post('/mob/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (!req.body.name || !req.body.name_en || !req.body.kind || !req.body.size) {
            res.send(new Response_1.ResponseError('', '名称、英文名称、类型和大小不能为空'));
            return;
        }
        // 处理版本数组：支持 number[] 或单个 number
        const versionIds = req.body.version
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, sqlArray)
            ...
            if (versionIds.length > 0) {
                ...
                (0, utility_1.getDataFromDB)(DB_NAME, relationSqls)
                    .then(() => {
                    (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
                    res.send(new Response_1.ResponseSuccess({ id: mobId }, '添加成功'));
                })
                    .catch(err => {
                    res.send(new Response_1.ResponseError(err, '添加版本关系失败'));
                });
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, sqlArray)
            ...
            if (versionIds.length > 0) {
                ...
                (0, utility_1.getDataFromDB)(DB_NAME, relationSqls)
                    .then(() => {
                    (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
                    res.send(new Response_1.ResponseSuccess({ id: plantId }, '添加成功'));
                })
                    .catch(err => {
                    res.send(new Response_1.ResponseError(err, '添加版本关系失败'));
                });
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, sqlArray)
            ...
            if (versionIds.length > 0) {
                ...
                (0, utility_1.getDataFromDB)(DB_NAME, relationSqls)
                    .then(() => {
                    (0, utility_1.updateUserLastLoginTime)(userInfo.uid);
                    res.send(new Response_1.ResponseSuccess({ id: thingId }, '添加成功'));
                })
                    .catch(err => {
                    res.send(new Response_1.ResponseError(err, '添加版本关系失败'));
                });
...
router.get('/version/list', (req, res) => {
    ...
    if (usePagination) {
        ...
        Promise.all(promisesAll)
            .then((_a) => __awaiter(void 0, [_a], void 0, function* ([dataList, dataSum]) {
            const enrichedList = (dataList || []).map((item) => {
                const enriched = Object.assign({}, item);
                convertIsActive(enriched);
                return enriched;
        ...
        (0, utility_1.getDataFromDB)(DB_NAME, [
            ...
            if (data) {
                const enrichedList = data.map((item) => {
                    const enriched = Object.assign({}, item);
                    convertIsActive(enriched);
                    return enriched;
                });
...
router.post('/version/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (!req.body.code || !req.body.name) {
            res.send(new Response_1.ResponseError('', '版本代码和名称不能为空'));
            return;
        }
        let sqlArray = [];
        sqlArray.push(`
            ...
            `);
        (0, utility_1.operate_db_and_return_added_id)(userInfo.uid, DB_NAME, DATA_NAME_VERSION, sqlArray, '添加', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
...
router.put('/version/modify', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        sqlArray.push(`
                ...
                WHERE id = ${req.body.id}
            `);
        (0, utility_1.operate_db_without_return)(userInfo.uid, DB_NAME, DATA_NAME_VERSION, sqlArray, '修改', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
...
router.delete('/version/delete', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.operate_db_without_return)(userInfo.uid, DB_NAME, DATA_NAME_VERSION, sqlArray, '删除', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
...
router.post('/craft-tab/add', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        .then(userInfo => {
        if (!req.body.code || !req.body.name) {
            res.send(new Response_1.ResponseError('', '标签代码和名称不能为空'));
            return;
        }
        let sqlArray = [];
        sqlArray.push(`
            ...
            `);
        (0, utility_1.operate_db_and_return_added_id)(userInfo.uid, DB_NAME, DATA_NAME_CRAFT_TAB, sqlArray, '添加', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
...
router.put('/craft-tab/modify', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        sqlArray.push(`
                ...
                WHERE id = ${req.body.id}
            `);
        (0, utility_1.operate_db_without_return)(userInfo.uid, DB_NAME, DATA_NAME_CRAFT_TAB, sqlArray, '修改', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
...
router.delete('/craft-tab/delete', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.operate_db_without_return)(userInfo.uid, DB_NAME, DATA_NAME_CRAFT_TAB, sqlArray, '删除', res);
    })
        .catch(errInfo => {
        res.send(new Response_1.ResponseError('', errInfo));
    });
