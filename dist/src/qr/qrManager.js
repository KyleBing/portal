...
router.get('/list', (req, res) => {
    (0, utility_1.verifyAuthorization)(req)
        ...
        (0, utility_1.getDataFromDB)('diary', sqlArray)
            ...
            data.forEach(qr => {
                // decode unicode
                qr.message = (0, utility_1.unicodeDecode)(qr.message);
                qr.description = (0, utility_1.unicodeDecode)(qr.description);
            });
        ...
        .catch(() => {
        res.send(new Response_1.ResponseError('无权查看 QR 列表：用户信息错误'));
    });
});
router.get('/detail', (req, res) => {
    ...
    (0, utility_1.getDataFromDB)('diary', sqlArray, true)
        ...
        if (dataQr.is_public === 1) {
            ...
            res.send(new Response_1.ResponseSuccess(dataQr));
        }
        else {
            ...
            (0, utility_1.verifyAuthorization)(req)
                ...
                .catch(() => {
                res.send(new Response_1.ResponseError('', '当前用户无权查看该 QR ：用户信息错误'));
            });
        }
    })
        .catch(err => {
    ...
    (0, utility_1.verifyAuthorization)(req)
        ...
        checkHashExist(req.body.hash)
            ...
            if (existLogs.length > 0) {
                // 2.1 已存在名为 hash 的记录
                res.send(new Response_1.ResponseError('', `已存在名为 ${req.body.hash} 的记录`));
            }
            else {
                // 2.2 不存在名为 hash 的记录
                let sqlArray = [];
                let parsedMessage = (0, utility_1.unicodeEncode)(req.body.message); // !
                let parsedDescription = (0, utility_1.unicodeEncode)(req.body.description) || '';
            ...
            .catch(err => {
            res.send(new Response_1.ResponseError(err, '查询 hash 记录出错'));
        });
    })
        .catch(err => {
...
router.post('/clear-visit-count', (req, res) => {
    ...
    (0, utility_1.verifyAuthorization)(req)
        ...
        if (userInfo.group_id === 1) {
            ...
            (0, utility_1.operate_db_and_return_added_id)(userInfo.uid, DB_NAME, DATA_NAME, sqlArray, '计数清零', res);
        }
        else {
